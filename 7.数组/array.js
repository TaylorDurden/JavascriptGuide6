...
返回第一个参数指定位置开始到第二个参数指定位置的前一个位置的所有元素。
...
不同于slice()和concat()，splice()会修改调用的数组。注意splice()和slice()有非常相似的名字，但它们的功能却有本质的区别。
splice()能从数组中删除元素、插入元素到数组中或同时完成这两种操作。在插入或删除点之后的数组元素会根据需要增加或减小它们的索引值，因此数组的其他部分仍保持连续的。

splice()的第一个参数指定了插入和(或)删除的起始位置。第二个参数指定了应该从数组中删除的元素的个数。如果省略第二个参数，从起始点开始到数组结尾的所有元素都将被删除。
